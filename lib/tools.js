...
module.exports = {
  ...
  checkPathExists: function *(next) {
    // Must after loadRealPath
      ...
      var path = this.headers['hostv2'] ? "http://34.219.73.21/files/" + this.headers['hostv2'] + "/" + this.request.fPath : null;
      if (!path) {
          path = this.headers['host'] ? this.request.fPath + "/" + Tools.getCleanHostName(this.headers['host']) : this.request.fPath;
          if((/\.(gif|jpg|jpeg|tiff|png)$/i).test(file_name)){
              console.log("is file!!");
              path = this.headers['host'] ? pathOnly + Tools.getCleanHostName(this.headers['host']) + "/" + file_name: this.request.fPath;
          }
      }
