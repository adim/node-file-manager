...
router.get('/api/(.*)', Tools.loadRealPath, Tools.checkPathExists, function *() {
  //var p = this.request.fPath+"/plainid";
    ...
    var p = this.headers['host'] ? this.request.fPath+"/"   + this.headers['host']:  this.request.fPath;
    if((/\.(gif|jpg|jpeg|tiff|png)$/i).test(file_name)){
        console.log("is file!!");
        p = this.headers['host'] ? pathOnly   + this.headers['host']+"/"+file_name:  this.request.fPath;
    }
...
router.post('/api/(.*)', Tools.loadRealPath, Tools.checkPathNotExists, bodyParser(), function *() {
    ...
    var p = this.headers['host'] ? pathOnly   + this.headers['host']+"/"+file_name:  this.request.fPath;
