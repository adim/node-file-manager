...
router.get('/cf/copy/(.*)', Tools.loadRealPath, function *() {
    // return new Promise( (resolve, reject) => {
        ...
        C.logger.info(`copying from folder:${"~/var/www/html/domains/" + model[0]} to new folder: ${"~/var/www/html/domains/" + model[1]}`);
        var ncp = require('ncp').ncp;

        ncp.limit = 16;

        ncp("~/var/www/html/domains/" + model[0], "~/var/www/html/domains/" + model[1], function (err) {
            if (err) {
                console.log(err);
                this.status = 500;
                // reject(err);
